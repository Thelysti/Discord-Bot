                    ...
                    if (code) {
                        Schema2.findOne({ Guild: message.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                client.embed({
                                    title: `${client.emotes.normal.error}・Модератор`,
                                    desc: `Ссылки на Discord на этом сервере запрещены!`,
                        ...
                        Schema2.findOne({ Guild: newMessage.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                let error = new Discord.EmbedBuilder()
                                    .setTitle(`${client.emotes.normal.error}・Модератор`)
                                    .setAuthor(client.user.username, client.user.avatarURL())
                                    .setDescription(`Ссылки на Discord на этом сервере запрещены!`)
